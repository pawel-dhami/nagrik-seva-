...
// ensure there is at least one authority account for testing
...
db.get('SELECT * FROM authorities WHERE email = ?', [DEFAULT_AUTH_EMAIL], (err, row) => {
  if (err) {
    console.error('error checking authorities table', err);
    return;
  }
  if (!row) {
...
const sessions = {}; // token -> { type, userId }
...
// citizen login/verification endpoint
// simple flow: accept OTP '123456' always
...
// report submission (citizen only) with optional photos
...
// authority can fetch all reports (photos parsed)
